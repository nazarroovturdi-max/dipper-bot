...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Привет! Я Диппер. Напиши мне обычный вопрос или укажи время и задачу (например: 18:30 выпить воды), и я напомню!")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ И СТАБИЛЬНЫЙ КЛИЕНТ GROQ
    try:
        ...
        await update.message.reply_text(f"⚙️ Ошибка связи с ИИ: {str(e)[:100]}")
